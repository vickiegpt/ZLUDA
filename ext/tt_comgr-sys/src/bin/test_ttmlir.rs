...
fn test_version_info() {
    ...
    let result = intel_comgr_get_version(&mut major, &mut minor);
    if result.is_ok() {
...
fn test_mlir_processing() {
    ...
    unsafe {
        ...
        if intel_comgr_create_data_set(&mut input_set).is_err() {
        ...
        if intel_comgr_create_data_set(&mut output_set).is_err() {
            println!("Failed to create output data set");
            intel_comgr_release_data_set(input_set).ok();
            return;
        }
        
        // Create source data for MLIR file
        let mut mlir_data = intel_comgr_data_t { handle: 0 };
        if intel_comgr_create_data(
            intel_comgr_data_kind_s::INTEL_COMGR_DATA_KIND_SOURCE,
            &mut mlir_data,
        ).is_err() {
            println!("Failed to create MLIR data");
            intel_comgr_release_data_set(input_set).ok();
            intel_comgr_release_data_set(output_set).ok();
            return;
        }
        
        // Set MLIR file name
        let mlir_filename = CString::new("test_example.mlir").unwrap();
        if intel_comgr_data_set_name(mlir_data, mlir_filename.as_ptr()).is_err() {
            println!("Failed to set MLIR data name");
            intel_comgr_release_data(mlir_data).ok();
            intel_comgr_release_data_set(input_set).ok();
            intel_comgr_release_data_set(output_set).ok();
            return;
        }
        ...
        if intel_comgr_data_set_add(input_set, mlir_data).is_err() {
        ...
        if intel_comgr_create_action_info(&mut action_info).is_err() {
        ...
        if intel_comgr_action_info_set_language(
            action_info,
            intel_comgr_language_s::INTEL_COMGR_LANGUAGE_SYCL,
        ).is_err() {
        ...
        let result = intel_comgr_do_action(
        ...
        if result.is_ok() {
            ...
            if intel_comgr_get_data_count(output_set, &mut output_count).is_ok() {
                println!("Generated {} output files", output_count);
                
                // List output files
                for i in 0..output_count {
                    let mut output_data = intel_comgr_data_t { handle: 0 };
                    if intel_comgr_get_data(output_set, i, &mut output_data).is_ok() {
                        let mut name_size = 0;
                        if intel_comgr_get_data_name(output_data, &mut name_size, ptr::null_mut()).is_ok() && name_size > 1 {
                            let mut name_buffer = vec![0i8; name_size];
                            if intel_comgr_get_data_name(output_data, &mut name_size, name_buffer.as_mut_ptr()).is_ok() {
        ...
        // Clean up
        intel_comgr_release_action_info(action_info).ok();
        intel_comgr_release_data(mlir_data).ok();
        intel_comgr_release_data_set(input_set).ok();
        intel_comgr_release_data_set(output_set).ok();
    }
